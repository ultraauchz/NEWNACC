...
foreach ($ministry as $key => $value) :
	$ministry_list[] = array('min_title' => $value, 'min_sum_project' => ($key+1)*10, 'project_not_announce' => ($key+1)*2);
endforeach;
?>
	...
	<table id="dynamic-table" class="table table-striped table-bordered table-hover dataTable no-footer DTTT_selectable" role="grid" aria-describedby="dynamic-table_info">
		<thead>
			<tr role="row">
				...
				<th class="sorting_disabled" tabindex="0" aria-controls="dynamic-table" rowspan="1" colspan="1" >จำนวนโครงการที่เผยแพร่ราคากลาง</th>
				<th class="sorting_disabled" tabindex="0" aria-controls="dynamic-table" rowspan="1" colspan="1" >จำนวนโครงการที่ไม่เผยแพร่ราคากลาง</th>
			</tr>
		</thead>
		...
		<tbody>
			<?php
				foreach ($ministry_list as $key => $value) :
					...
					echo '<td class="right">'.number_format($value['project_not_announce']).'</td>';
				endforeach;
			?>
		</tbody>
	</table>
...
<script type="text/javascript">
	$(function () {
		$('#highcharts').highcharts({
			...
			title: {
				text: 'กราฟสรุปจำนวนโครงการที่มีและไม่มีการประกาศราคากลาง'
			},
			/*
			subtitle: {
			...
			*/
			...
			series: [{
				name: 'จำนวนโครงการที่เผยแพร่ราคากลาง',
				data: [
					<?php
						foreach ($ministry_list as $key => $value) :
							echo $value['min_sum_project'].', ';
						endforeach;
					?>
					]
			},
			{
				name: 'จำนวนโครงการที่ไม่เผยแพร่ราคากลาง',
				data: [
					<?php
						foreach ($ministry_list as $key => $value) :
							echo $value['project_not_announce'].', ';
						endforeach;
					?>
					]
			}
